...
var A1 = spark.sql("select * from usfinance.aml_kyds_mainTB_withliushui where tr_tm is not null").
groupBy("acct_no").agg(count(lit(1)).alias("tr_cnt"))
val B = spark.sql("""
...
A = A.join(B, Seq("acct_no"), "left").join(A1,Seq("acct_no"),"left").
withColumn("kyds1", when($"kyds1".isNotNull && $"tr_cnt" > 30, 10.0).otherwise(0.0))
